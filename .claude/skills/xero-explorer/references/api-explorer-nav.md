...
```bash
agent-browser --auto-connect snapshot -i          # Capture page accessibility tree (use -i for concise output)
...
agent-browser --auto-connect find role button click --name "TEXT"         # Find a button by accessible name and click it
agent-browser --auto-connect find role button click --name "TEXT" --exact # Same but exact-match only (no substring)
...
All commands require `--auto-connect` so the user can observe and intervene.

## Session Setup

```bash
# Open API Explorer (use --auto-connect so user can see/intervene)
agent-browser --auto-connect open "https://api-explorer.xero.com/"
agent-browser --auto-connect wait 3000

# Verify logged in -- tenant selector should be visible
agent-browser --auto-connect snapshot -i | grep "Tenant"
```
...
```bash
# pseudo-flow
agent-browser --auto-connect snapshot -i > /tmp/snap.txt
...
agent-browser --auto-connect click @ENDPOINT_BUTTON_REF
agent-browser --auto-connect wait 500
agent-browser --auto-connect find role button click --name "Select endpoint NEW_ENDPOINT" --exact
...
agent-browser --auto-connect find text "Get Bank Transactions" click

# GOOD: exact match
agent-browser --auto-connect find role button click --name "Select operation Get Bank Transactions" --exact
```
