#!/usr/bin/env python3
"""Move SAGUFT to 911 (Loan - Nathan) and ZipMoney to 485 (Software)."""
import csv
import json
...
for i, row in enumerate(rows):
    ...
    if "SAGUFT" in payee.upper():
        row["AccountCode"] = "911"
        row["AccountName"] = accounts.get("911", "")
        row["Status"] = "APPROVE"
        fixes.append(f"L{line}: SAGUFT ({amount}) -> 911 Loan - Nathan")

    if "ZIPMONEY" in payee.upper():
        row["AccountCode"] = "485"
        row["AccountName"] = accounts.get("485", "")
        fixes.append(f"L{line}: ZipMoney ({amount}) -> 485 Software")
